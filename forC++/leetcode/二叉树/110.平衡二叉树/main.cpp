...
public:
    int getHeight(TreeNode* root) {
        // 求高度，用后序
        if (NULL == root) return 0;             // 遇到空节点时返回高度0
        int height_l = getHeight(root->left);   // 遍历左子树
        if (-1 == height_l) return -1;          // 置标志位后快速返回
        int height_r = getHeight(root->right);  // 遍历右子树
        if (-1 == height_r) return -1;          // 置标志位后快速返回
        if (abs(height_l - height_r) > 1) {     // 若左右子树高度差绝对值>1
            return -1;                          // 开始快速返回
        }      
    ...
    bool isBalanced(TreeNode* root) {
        if (-1 == getHeight(root)) return false;
        return true;
    }
};
...
int main() {
    ...
    vector<int> vals = {1,2,2,3,3,null,null,4,4};   // NULL_FLAG代表空
    TreeNode *t = tree_build_level(&vals);
    tree_traversal_level(t);

    bool result = s.isBalanced(t);
