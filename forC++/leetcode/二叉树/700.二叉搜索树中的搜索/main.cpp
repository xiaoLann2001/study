...
public:
    TreeNode* searchBST(TreeNode* root, int val) {
        if (root->val == val) return root;  // 终止条件，当找到val时，返回根
        // 当val<根时，若左子树存在，在左子树中找
        if (val < root->val && root->left) return searchBST(root->left, val);
        // 当val>根时，若右子树存在，在右子树中找
        if (val > root->val && root->right) return searchBST(root->right, val);
        return NULL;    // 子树的根和左右子树都没有找到，返回空
    }
};
...
public:
    TreeNode* searchBST(TreeNode* root, int val) {
        while (root->val != val) {
            ...
            else if (val > root->val && root->right) root = root->right;
            else return NULL;
        }
        return root;
    }
...
int main() {
    ...
    TreeNode *result = s.searchBST(t, val);
