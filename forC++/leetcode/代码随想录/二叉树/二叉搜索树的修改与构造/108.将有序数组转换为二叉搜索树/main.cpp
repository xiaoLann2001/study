...
public:
    ...
    TreeNode* build(vector<int>& nums, int begin, int end) {
        ...
        root->left = build(nums, begin, begin + (end - begin) / 2);
        root->right = build(nums, begin + (end - begin) / 2 + 1, end);
        return root;
    }
};
...
int main() {
    ...
    vector<int> vals = {-10,-3,0,5,9};  // 有序数组

    TreeNode* result = s.sortedArrayToBST(vals);
