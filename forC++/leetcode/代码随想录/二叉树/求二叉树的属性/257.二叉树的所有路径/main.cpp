...
public:
    ...
    void preorder_traversal(TreeNode* root, string prelevel) {
        // 从根到叶子算一条路径，用先序遍历
        // if (NULL == root) return;
        string str = prelevel + to_string(root->val);
        if (!root->left && !root->right)    // 中若为叶子，加到末尾并加入到结果集
            result.push_back(str);
        else {
            if (root->left) // 避免访问空节点
                preorder_traversal(root->left, str + "->");    // 左
            if (root->right) 
                preorder_traversal(root->right, str + "->");   // 右
        }
    }
...
int main() {
    ...
    vector<int> vals = {1,2,3,null,5};   // NULL_FLAG代表空
    TreeNode *t = tree_build_level(&vals);
    tree_traversal_level(t);

    vector<string> result = s.binaryTreePaths(t);
