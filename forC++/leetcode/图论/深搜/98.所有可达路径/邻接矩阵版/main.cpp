#include <bits/stdc++.h>

using namespace std;

// 邻接矩阵 vector<vector<int>>
// 邻接表 vector<list<int>>

class Solution {
public:
    vector<vector<int>> result;
    vector<int> path;

    void backtracking(vector<vector<int>> &graph, int s) {
        // 终止条件
        if (s == graph.size() - 1) {  // 若到达终点 n, 记录 path，并返回
            result.push_back(path);
            return;
        }
        // 单层递归逻辑
        for (int t = 1; t < graph[s].size(); t++) { // 遍历当前节点指向的节点
            if (graph[s][t] == 1) {
                path.push_back(t);  // 处理节点
                backtracking(graph, t); // 递归当前节点指向的节点
                path.pop_back();    // 回溯
            }
        }
    } 

    vector<vector<int>> dfs(vector<vector<int>> &graph) {
        result.clear();
        path.clear();
        path.push_back(1);
        backtracking(graph, 1);
        return result;
    }
};

int main() {
    // input and build graph
    int n, m, s, t;
    cin >> n >> m;
    // vector<vector<int>> graph(n + 1, vector<int>(m + 1, 0));
    vector<vector<int>> graph(n + 1, vector<int>(n + 1, 0));    // 邻接矩阵 (n * n)
    while (m--) {
        cin >> s >> t;
        graph[s][t] = 1;
    }

    // solve problem and output
    Solution sl;
    vector<vector<int>> result;
    result = sl.dfs(graph);
    if (result.empty()) cout << "-1" << endl;
    else {
        for (int i = 0; i < result.size(); i++) {
            for (int j = 0; j < result[i].size() - 1; j++) cout << result[i][j] << " ";
            cout << result[i][result[i].size() - 1] << endl;
        }
    }

    return 0;
}