...
void Serialport::handleTimeout()
{
    ...
    if (buffer.contains(frameHeader) && buffer.contains(frameFooter))
    {
        ...
        for (size_t i = 0; i < sizeof(rfs.charArray); ++i)
        {
            rfs.charArray[i] = frameData.at(static_cast<int>(i) + 2); // 帧头占2位
        }

        QString sensorData = QString{"Pitch = %1, Roll = %2, Yaw = %3, EA = %4, EB = %5"}
                                         .arg(static_cast<double>(rfs.stc.Pitch), 0, 'f', 4)
                                         .arg(static_cast<double>(rfs.stc.Roll), 0, 'f', 4)
                                         .arg(static_cast<double>(rfs.stc.Yaw), 0, 'f', 4)
                                         .arg(static_cast<double>(rfs.stc.EA))
                                         .arg(static_cast<double>(rfs.stc.EB));
